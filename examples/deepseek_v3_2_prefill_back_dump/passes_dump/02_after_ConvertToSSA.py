    ...
    def deepseek_v3_2_prefill_back_layer(self, hidden_states_0: pl.Tensor[[16, 4096, 7168], pl.BFLOAT16], seq_lens_0: pl.Tensor[[16], pl.INT32], node_id_t_0: pl.Tensor[[1], pl.INT32], combine_buf_0: pl.Tensor[[128, 16, 4096, 16384], pl.BFLOAT16], wo_0: pl.Tensor[[16384, 7168], pl.BFLOAT16], post_rms_weight_0: pl.Tensor[[1, 7168], pl.FP32], w_gate_0: pl.Tensor[[7168, 18432], pl.BFLOAT16], w_up_0: pl.Tensor[[7168, 18432], pl.BFLOAT16], w_down_0: pl.Tensor[[18432, 7168], pl.BFLOAT16], out_0: pl.Tensor[[16, 4096, 7168], pl.BFLOAT16]) -> pl.Tensor[[16, 4096, 7168], pl.BFLOAT16]:
        ...
        for b_0, (out_iter_1,) in pl.parallel(0, 16, 1, init_values=(out_0,), chunk=4):
            ...
            for p0_idx_0, (out_iter_3,) in pl.range(0, tok_blocks_0, 1, init_values=(out_iter_1,)):
                ...
                with pl.auto_incore():
                    combined_tile_0: pl.Tensor[[4, 16384], pl.FP32] = pl.tensor.cast(pl.tensor.view(combine_buf_0, [4, 16384], [node_id_0, b_0, p0_0, 0]), target_type=pl.FP32, mode=2)
                    ...
                    for ob_0, (resid1_tile_iter_1,) in pl.parallel(0, 56, 1, init_values=(resid1_tile_0,), chunk=8):
                        ...
                        for kb_0, (o_acc_iter_2,) in pl.range(0, 32, 1, init_values=(o_acc_1,)):
                            ...
                            a_chunk_0: pl.Tensor[[4, 512], pl.BFLOAT16] = pl.tensor.cast(pl.tensor.view(combined_tile_0, [4, 512], [0, k0_0]), target_type=pl.BFLOAT16, mode=2)
                            w_chunk_0: pl.Tensor[[512, 128], pl.BFLOAT16] = pl.tensor.view(wo_0, [512, 128], [k0_0, o0_0])
                            o_acc_4: pl.Tensor[[4, 128], pl.FP32] = pl.tensor.add(o_acc_iter_2, pl.tensor.matmul(a_chunk_0, w_chunk_0, a_trans=False, b_trans=False, c_matrix_nz=False))
                            o_acc_3: pl.Tensor[[4, 128], pl.FP32] = pl.yield_(o_acc_4)
                        resid_0: pl.Tensor[[4, 128], pl.FP32] = pl.tensor.cast(pl.tensor.view(hidden_states_0, [4, 128], [b_0, p0_0, o0_0]), target_type=pl.FP32, mode=2)
                        resid1_tile_3: pl.Tensor[[4, 7168], pl.FP32] = pl.tensor.assemble(resid1_tile_iter_1, pl.tensor.add(o_acc_3, resid_0), [0, o0_0])
                    ...
                    for kb_2, (k0_iter_4, post_norm_tile_iter_1, x_chunk_iter_1) in pl.range(0, 14, 1, init_values=(k0_2, post_norm_tile_0, x_chunk_0)):
                        ...
                        normed_0: pl.Tensor[[4, 512], pl.FP32] = pl.tensor.col_expand_mul(pl.tensor.row_expand_mul(x_chunk_3, inv_rms_0), gamma_0)
                        post_norm_tile_3: pl.Tensor[[4, 7168], pl.BFLOAT16] = pl.tensor.assemble(post_norm_tile_iter_1, pl.tensor.cast(normed_0, target_type=pl.BFLOAT16, mode=2), [0, k0_6])
                    ...
                    for ob_1, (down_proj_tile_iter_2, k0_iter_7, kb_iter_3, o0_iter_1) in pl.range(0, 36, 1, init_values=(down_proj_tile_1, k0_5, kb_2, o0_0)):
                        ...
                        for kb_5, (gate_acc_iter_2, k0_iter_9, up_acc_iter_2) in pl.range(0, 14, 1, init_values=(gate_acc_1, k0_iter_7, up_acc_1)):
                            ...
                            gate_acc_4: pl.Tensor[[4, 512], pl.FP32] = pl.tensor.add(gate_acc_iter_2, pl.tensor.matmul(post_chunk_0, wg_0, a_trans=False, b_trans=False, c_matrix_nz=False))
                            up_acc_4: pl.Tensor[[4, 512], pl.FP32] = pl.tensor.add(up_acc_iter_2, pl.tensor.matmul(post_chunk_0, wu_0, a_trans=False, b_trans=False, c_matrix_nz=False))
                            gate_acc_3, k0_10, up_acc_3 = pl.yield_(gate_acc_4, k0_11, up_acc_4)
                        sigmoid_0: pl.Tensor[[4, 512], pl.FP32] = pl.tensor.recip(pl.tensor.add(pl.tensor.exp(pl.tensor.neg(gate_acc_3)), 1.0))
                        mlp_chunk_0: pl.Tensor[[4, 512], pl.FP32] = pl.tensor.mul(pl.tensor.mul(gate_acc_3, sigmoid_0), up_acc_3)
                        ...
                        for dob_0, (down_proj_tile_iter_4,) in pl.parallel(0, 56, 1, init_values=(down_proj_tile_iter_2,), chunk=8):
                            ...
                            down_next_0: pl.Tensor[[4, 128], pl.FP32] = pl.tensor.add(down_prev_0, pl.tensor.matmul(mlp_chunk_bf16_0, w_down_chunk_0, a_trans=False, b_trans=False, c_matrix_nz=False))
                    ...
                    for ob_2, (o0_iter_4, out_iter_5) in pl.parallel(0, 56, 1, init_values=(o0_2, out_iter_3), chunk=8):
                        o0_6: pl.Scalar[pl.INDEX] = ob_2 * 128
                        down_acc_0: pl.Tensor[[4, 128], pl.FP32] = pl.tensor.add(pl.tensor.view(down_proj_tile_3, [4, 128], [0, o0_6]), pl.tensor.view(resid1_tile_2, [4, 128], [0, o0_6]))
                        out_7: pl.Tensor[[16, 4096, 7168], pl.BFLOAT16] = pl.tensor.assemble(out_iter_5, pl.tensor.cast(down_acc_0, target_type=pl.BFLOAT16, mode=2), [b_0, p0_0, o0_6])
