            ...
            for p0_idx in pl.range(0, tok_blocks, 1):
                ...
                with pl.auto_incore():
                    ...
                    for kb in pl.range(0, 20, 1):
                        ...
                        x_chunk: pl.Tensor[[4, 256], pl.FP32] = pl.tensor.cast(pl.tensor.view(hidden_states, [4, 256], [b, p0, k0]), target_type=pl.FP32, mode=2)
                    ...
                    q_proj_tile: pl.Tensor[[4, 5120], pl.BFLOAT16] = pl.tensor.create([4, 5120], dtype=pl.BFLOAT16)
                    k_proj_tile: pl.Tensor[[4, 1024], pl.BFLOAT16] = pl.tensor.create([4, 1024], dtype=pl.BFLOAT16)
                    v_proj_tile: pl.Tensor[[4, 1024], pl.BFLOAT16] = pl.tensor.create([4, 1024], dtype=pl.BFLOAT16)
                    for ob in pl.parallel(0, 80, 1, chunk=8):
                        ...
                        q_proj_tile: pl.Tensor[[4, 5120], pl.BFLOAT16] = pl.tensor.assemble(q_proj_tile, pl.tensor.cast(q_acc, target_type=pl.BFLOAT16, mode=2), [0, q0])
                    for ob in pl.parallel(0, 32, 1, chunk=8):
                        ...
                        k_proj_tile: pl.Tensor[[4, 1024], pl.BFLOAT16] = pl.tensor.assemble(k_proj_tile, pl.tensor.cast(k_acc, target_type=pl.BFLOAT16, mode=2), [0, kv0])
                        v_proj_tile: pl.Tensor[[4, 1024], pl.BFLOAT16] = pl.tensor.assemble(v_proj_tile, pl.tensor.cast(v_acc, target_type=pl.BFLOAT16, mode=2), [0, kv0])
                with pl.auto_incore():
                    ...
                    for ti in pl.range(0, valid_tok, 1):
                        ...
                        for h in pl.parallel(0, 64, 1, chunk=8):
                            ...
                            if h % 8 == 0:
                                ...
                                k_row: pl.Tensor[[1, 128], pl.FP32] = pl.tensor.cast(pl.tensor.view(k_proj_tile, [1, 128], [ti, kv_col]), target_type=pl.FP32, mode=2)
                                ...
                                v_cache: pl.Tensor[[524288, 128], pl.BFLOAT16] = pl.tensor.assemble(v_cache, pl.tensor.view(v_proj_tile, [1, 128], [ti, kv_col]), [cache_row, 0])
                            q_row: pl.Tensor[[1, 128], pl.FP32] = pl.tensor.cast(pl.tensor.view(q_proj_tile, [1, 128], [ti, q_col]), target_type=pl.FP32, mode=2)
                            ...
                            for sb in pl.range(0, ctx_blocks, 1):
                                ...
                                valid_len: pl.Scalar[pl.INDEX] = min(120, ctx_len - s0)
                ...
                with pl.auto_incore():
                    ...
                    for ob in pl.parallel(0, 80, 1, chunk=8):
                        ...
                        for kb in pl.range(0, 20, 1):
                            k0: pl.Scalar[pl.INDEX] = kb * 256
                            a_chunk: pl.Tensor[[4, 256], pl.BFLOAT16] = pl.tensor.cast(pl.tensor.view(attn_tile, [4, 256], [0, k0]), target_type=pl.BFLOAT16, mode=2)
                        ...
                        resid: pl.Tensor[[4, 64], pl.FP32] = pl.tensor.cast(pl.tensor.view(hidden_states, [4, 64], [b, p0, o0]), target_type=pl.FP32, mode=2)
                    ...
                    for ob in pl.range(0, 100, 1):
                        o0: pl.Scalar[pl.INDEX] = ob * 256
                        gate_acc: pl.Tensor[[4, 256], pl.FP32] = pl.tensor.create([4, 256], dtype=pl.FP32)
                        up_acc: pl.Tensor[[4, 256], pl.FP32] = pl.tensor.create([4, 256], dtype=pl.FP32)
                        gate_acc: pl.Tensor[[4, 256], pl.FP32] = pl.tensor.mul(gate_acc, 0.0)
                        up_acc: pl.Tensor[[4, 256], pl.FP32] = pl.tensor.mul(up_acc, 0.0)
                        for kb in pl.range(0, 20, 1):
                            k0: pl.Scalar[pl.INDEX] = kb * 256
                            post_chunk: pl.Tensor[[4, 256], pl.BFLOAT16] = pl.tensor.view(post_norm_tile, [4, 256], [0, k0])
                            wg: pl.Tensor[[256, 256], pl.BFLOAT16] = pl.tensor.view(w_gate, [256, 256], [k0, o0])
                            wu: pl.Tensor[[256, 256], pl.BFLOAT16] = pl.tensor.view(w_up, [256, 256], [k0, o0])
                            gate_acc: pl.Tensor[[4, 256], pl.FP32] = pl.tensor.add(gate_acc, pl.tensor.matmul(post_chunk, wg, a_trans=False, b_trans=False, c_matrix_nz=False))
                            up_acc: pl.Tensor[[4, 256], pl.FP32] = pl.tensor.add(up_acc, pl.tensor.matmul(post_chunk, wu, a_trans=False, b_trans=False, c_matrix_nz=False))
                        sigmoid: pl.Tensor[[4, 256], pl.FP32] = pl.tensor.recip(pl.tensor.add(pl.tensor.exp(pl.tensor.neg(gate_acc)), 1.0))
                        mlp_chunk: pl.Tensor[[4, 256], pl.FP32] = pl.tensor.mul(pl.tensor.mul(gate_acc, sigmoid), up_acc)
                        mlp_chunk_bf16: pl.Tensor[[4, 256], pl.BFLOAT16] = pl.tensor.cast(mlp_chunk, target_type=pl.BFLOAT16, mode=2)
                        for dob in pl.parallel(0, 80, 1, chunk=8):
                            ...
                            w_down_chunk: pl.Tensor[[256, 64], pl.BFLOAT16] = pl.tensor.view(w_down, [256, 64], [o0, d0])
                            ...
                            if ob == 100 - 1:
                                down_acc: pl.Tensor[[4, 64], pl.FP32] = pl.tensor.add(pl.tensor.view(down_proj_tile, [4, 64], [0, d0]), pl.tensor.view(resid1_tile, [4, 64], [0, d0]))
                                out: pl.Tensor[[16, 4096, 5120], pl.BFLOAT16] = pl.tensor.assemble(out, pl.tensor.cast(down_acc, target_type=pl.BFLOAT16, mode=2), [b, p0, d0])
        return out